...
/*	 $("#btnImprimir"+elementos).click(function(e) {
	...
	$('#field9').on('change', function() {
		$('#field3').empty();
		$("#field3").append('<option value="" selected="true" >Seleccione un submenú...</option>');
		var idMenu = $(this).val();
		consultarSubmenusPorMenu(idMenu);
	});
	
	function consultarSubmenusPorMenu(idMenu){
		$.ajax({
			  url: "./config",
			  data: {
				  action: "consultarSubmenusPorMenu",
				  idMenu:  idMenu
			  },		  
			  success: function(result){
				  $.each(result, function(i,item){						
						$("#field3").append('<option value="'+item.idAction+'">'+item.nombreAction+'</option>');
					});
			  },
