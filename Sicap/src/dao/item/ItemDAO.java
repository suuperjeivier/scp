...
//				System.out.println(query);
	...
	public Vector<ItemDTO> selectItemsSinMovimiento(int idTipoMovimiento, int tipoItem){	
		Vector<ItemDTO> vItemTipo = new Vector<ItemDTO>();
		if(database.openDatabase()){
			System.out.println("conexion abierta en " + this.getClass().getSimpleName());
			ResultSet rs = null;
			String query= "SELECT * FROM tb_items\r\n" + 
					" WHERE tb_items.fk_id_item_tipo = "+tipoItem+" and NOT EXISTS (SELECT *\r\n" + 
					"                   FROM   tb_movimientos_items \r\n" + 
					"                   WHERE tb_movimientos_items.fk_id_item = tb_items.id_item) ";
			try {
				System.out.println(query);
				rs = database.executeQuery(query);
				if(rs != null){					
