...
public class MovimientosService {
	...
	public void inicializaJSPConsultaOrdenes(HttpServletRequest request, HttpServletResponse response){
		...
		Vector<Object> vTecnicosChoferes = new Vector<Object>();
		final String _VEHICULOS = "vehiculos";
		final String _RADIOS = "radios";
		final String _TECNICOS = "tecnicos";		
		final String _LUGARES = "lugares";
		vVehiculos = getOrdenesDAO().selectTablaFromOrdenesOrderByNombre(_VEHICULOS, false);//trae vehiculos
		vRadios = getOrdenesDAO().selectTablaFromOrdenesOrderByNombre(_RADIOS, false);//trae RADIOS
		vTecnicos = getOrdenesDAO().selectTablaFromOrdenesOrderByNombre(_TECNICOS, false);//trae TECNICOS
		vLugares = getOrdenesDAO().selectTablaFromOrdenesOrderByNombre(_LUGARES, false);//trae LUGARES
		vTecnicosChoferes = getOrdenesDAO().selectTablaFromOrdenesOrderByNombre(_TECNICOS, true);//trae TECNICOS chof
		if(vVehiculos != null){
		...
		if(vTecnicosChoferes != null){
			if(vTecnicosChoferes.size() > 0){
				request.setAttribute("tecnicosChoferes", vTecnicosChoferes);
			}else{
				System.out.println("el vector de vTecnicosChoferes es menor o igual  a 0");
			}
		}else{
			System.out.println("el vector de vTecnicosChoferes es menor o igual  a 0");
		}
		...
		if(orderId > 0){
			...
			if(vOLugares != null){
				if(vOLugares.size() > 0){
					...
					OrdenLugarDTO ol = new OrdenLugarDTO();
					ol.getOrdenDTO().setOrderId(orderId);
					vOLugares.add(ol);
					obj_arr.add(vOLugares);
				}
			}else{
				System.out.println("el vector de vOLugares es menor o igual  a 0");
				vOLugares = new Vector<>();
				OrdenLugarDTO ol = new OrdenLugarDTO();
				ol.getOrdenDTO().setOrderId(orderId);
				vOLugares.add(ol);
				obj_arr.add(vOLugares);
			...
			if(vOTecnicos != null){
				if(vOTecnicos.size() > 0){
					...
					OrdenTecnicoDTO ot = new OrdenTecnicoDTO();
					ot.getOrdenDTO().setOrderId(orderId);
					vOTecnicos.add(ot);
					obj_arr.add(vOTecnicos);
				}
			}else{
				System.out.println("el vector de vOTecnicos es menor o igual  a 0");
				vOTecnicos = new Vector<>();
				OrdenTecnicoDTO ot = new OrdenTecnicoDTO();
				ot.getOrdenDTO().setOrderId(orderId);
				vOTecnicos.add(ot);
				obj_arr.add(vOTecnicos);
			...
			if(vORadios != null){
				if(vORadios.size() > 0){
					...
					OrdenRadioDTO or = new OrdenRadioDTO();
					or.getOrdenDTO().setOrderId(orderId);
					vORadios.add(or);
					obj_arr.add(vORadios);
				}
			}else{
		...
		if(tipoMovimiento > 0){
			...
			if(vMovItem != null){
				if(vMovItem.size() > 0){
					...
					if(tipoMovimiento == 1){//por que es asignar un rfsi a un radio y solicitaron que se pusieran los chkbox en esa vista del form
						Vector<AccesorioDTO> accesorios = new Vector<>();
						accesorios = getMovimientosDAO().selectAccesoriosRadio();
	...
	public void procesaFormOrdenesService(HttpServletRequest request, HttpServletResponse response){
		...
		{	
			...
			if(ordenDTO.getVehiculoDTO().getVehiculoId() > 0)
			if(ordenDTO.getVehiculoDTO().getVehiculoId() == 4){				
				String campoNewVehicle = "fieldNewV2";
				if(request.getParameter(campoNewVehicle) != null && !request.getParameter(campoNewVehicle).isEmpty())
				{
					ordenDTO.getVehiculoDTO().setNombre(request.getParameter(campoNewVehicle).toUpperCase());
				}else{
					fieldsMissing += campoNewVehicle;
				}
				campoNewVehicle = "fieldNewV3";
				if(request.getParameter(campoNewVehicle) != null && !request.getParameter(campoNewVehicle).isEmpty())
				{
					ordenDTO.getVehiculoDTO().setTipo(request.getParameter(campoNewVehicle).toUpperCase());
				}else{
					fieldsMissing += campoNewVehicle;
				}
				campoNewVehicle = "fieldNewV4";
				if(request.getParameter(campoNewVehicle) != null && !request.getParameter(campoNewVehicle).isEmpty())
				{
					ordenDTO.getVehiculoDTO().setMarca(request.getParameter(campoNewVehicle).toUpperCase());
				}else{
					fieldsMissing += campoNewVehicle;
				}
				campoNewVehicle = "fieldNewV5";
				if(request.getParameter(campoNewVehicle) != null && !request.getParameter(campoNewVehicle).isEmpty())
				{
					ordenDTO.getVehiculoDTO().setLinea(request.getParameter(campoNewVehicle).toUpperCase());
				}else{
					fieldsMissing += campoNewVehicle;
				}
				campoNewVehicle = "fieldNewV6";
				if(request.getParameter(campoNewVehicle) != null && !request.getParameter(campoNewVehicle).isEmpty())
				{
					ordenDTO.getVehiculoDTO().setModelo(request.getParameter(campoNewVehicle).toUpperCase());
				}else{
					fieldsMissing += campoNewVehicle;
				}
				campoNewVehicle = "fieldNewV7";
				if(request.getParameter(campoNewVehicle) != null && !request.getParameter(campoNewVehicle).isEmpty())
				{
					ordenDTO.getVehiculoDTO().setPlacas(request.getParameter(campoNewVehicle).toUpperCase());
				}else{
					fieldsMissing += campoNewVehicle;
				}
				campoNewVehicle = "fieldNewV8";
				if(request.getParameter(campoNewVehicle) != null && !request.getParameter(campoNewVehicle).isEmpty())
				{
					ordenDTO.getVehiculoDTO().setInventario(request.getParameter(campoNewVehicle).toUpperCase());
				}else{
					fieldsMissing += campoNewVehicle;
				}
				campoNewVehicle = "fieldNewV9";
				if(request.getParameter(campoNewVehicle) != null && !request.getParameter(campoNewVehicle).isEmpty())
				{
					ordenDTO.getVehiculoDTO().setNumPolizaSeguro(request.getParameter(campoNewVehicle).toUpperCase());
				}else{
					fieldsMissing += campoNewVehicle;
				}
				ordenDTO.getVehiculoDTO().setActivo('S');
				int vehicleId = getOrdenesDAO().insertNewVehiculoDTOReturningLastId(ordenDTO.getVehiculoDTO());
				if(vehicleId > 0){
					ordenDTO.getVehiculoDTO().setVehiculoId(vehicleId);
				}else{
					System.out.println("FALLO AL INSERTAR UN NUEVO VEHICULO EXTERNO!");
					ordenDTO.getVehiculoDTO().setVehiculoId(vehicleId);
				}
			}else{
				//vehiculo registrado
		...
		if(request.getParameter(campo) != null && !request.getParameter(campo).isEmpty())
		{			
			ordenDTO.getTecnicoChoferDTO().setTecnicoId(Integer.parseInt(request.getParameter(campo)));
		}else{
			fieldsMissing += campo;
		}
		campo = "field9";
		if(request.getParameter(campo) != null && !request.getParameter(campo).isEmpty())
		{			
			ordenDTO.getVehiculoDTO().setNumPolizaSeguro(request.getParameter(campo));
		}else{
			fieldsMissing += campo;
		}
		...
		{				
			if(request.getParameterValues("fieldCommentsLugares[]") != null && request.getParameterValues("fieldCommentsLugares[]").length > 0)
			{
				...
				for (String lugar : lugaresCombo) {
					...
					System.out.println(ordenLugar.getActividad());
					...
					if(filePart != null){
						...
						if(fileName != null && !fileName.isEmpty()){	
							if(fileName.length()<=250){
								...
								System.out.print("Archivo con nombre largo");
							}
						}else{
	...
	public void selectItemsPorTipoItem(HttpServletRequest request, HttpServletResponse response){
		Vector<ItemDTO> vItems = new Vector<ItemDTO>();
		int tipoMovimiento = Integer.parseInt(request.getParameter("moveTypeId"));
		vItems = getItemDAO().selectItemsSinMovimiento(tipoMovimiento, 2);
		if(vItems != null){
			if(vItems.size() > 0){
				request.setAttribute("listado_itemRadio", vItems);
			}else{
				System.out.println("el vector de movimientoItem es menor o igual  a 0");
			}
		}else{
		...
		vItems = getItemDAO().selectItemsSinMovimiento(tipoMovimiento, 1);
		if(vItems != null){
			if(vItems.size() > 0){
				request.setAttribute("listado_vehiculos", vItems);
			}else{
				System.out.println("el vector de movimientoItem es menor o igual  a 0");
			}
		}else{
		...
		vItems = getItemDAO().selectItemsSinMovimiento(tipoMovimiento, 3);
		if(vItems != null){
			if(vItems.size() > 0){
				request.setAttribute("listado_cargadores", vItems);
			}else{
				System.out.println("el vector de movimientoItem es menor o igual  a 0");
			}
		}else{
		...
		vItems = getItemDAO().selectItemsSinMovimiento(tipoMovimiento, 6);
		if(vItems != null){
			if(vItems.size() > 0){
				request.setAttribute("listado_gpss", vItems);
			}else{
				System.out.println("el vector de movimientoItem es menor o igual  a 0");
			}
		}else{
	...
	public void selectMovimientosItemsTodos(HttpServletRequest request, HttpServletResponse response){
		...
		int tipoMovimiento = Integer.parseInt(request.getParameter("moveTypeId"));
		vItemRad = getItemDAO().selectItemsSinMovimiento(tipoMovimiento, 2);
		if(vItemRad != null){
			if(vItemRad.size() > 0){
	...
	public void selectMovimientosItemsPorAsignar(HttpServletRequest request, HttpServletResponse response){
		...
		int tipoMovimiento = Integer.parseInt(request.getParameter("moveTypeId"));
		int tipoItemRadio = 2;
		vItemRad = getItemDAO().selectItemsSinMovimiento(tipoMovimiento, tipoItemRadio);
		if(vItemRad != null){
			if(vItemRad.size() > 0){
				request.setAttribute("listado_itemRadio", vItemRad);
			}else{
