...
public class ConfigController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(user != null){
			...
			if(action != null && !action.equals("") && action.length() > 0){
				...
				}else if(action.equals("consultarSubmenusPorMenu")){
					consultarSubmenusPorMenu(request,response);
				}else{
	...
	public void consultarSubmenusPorMenu(HttpServletRequest request, HttpServletResponse response){
		getConfigService().consultarSubmenusServicePorMenu(request, response);		
	}
	...
	public void menuPerfil(HttpServletRequest request, HttpServletResponse response){
		//getConfigService().consultarSubmenusService(request, response);//cambiar por menu
		getConfigService().consultarMenusService(request, response);
		getConfigService().consultarPerfilesService(request, response);
